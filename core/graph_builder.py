...
graph_builder.add_edge('agent_b', 'human_review', condition=lambda state: state['data_store'].get('decision', 'human_review') == 'human_review')
graph_builder.add_edge('human_review', 'stream_output', condition=lambda state: state['data_store'].get('approved', False))
graph_builder.add_edge('human_review', 'error_handler', condition=lambda state: not state['data_store'].get('approved', False))
